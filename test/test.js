...
async function runTests() {
  ...
  try {
    ...
    console.log(`  - Always rules: ${categorizedRules[RuleType.ALWAYS].length}`);
    console.log(`  - Agent Requested rules: ${categorizedRules[RuleType.AGENT_REQUESTED].length}`);
    console.log(`  - Auto Attached rules: ${categorizedRules[RuleType.AUTO_ATTACHED].length}`);
    console.log(`  - Manual rules: ${categorizedRules[RuleType.MANUAL].length}`);
    ...
    if (categorizedRules[RuleType.ALWAYS].length > 0 && 
        !claudeContent.includes('프로젝트에서 반드시 아래 파일의 내용을 참고하세요:')) {
